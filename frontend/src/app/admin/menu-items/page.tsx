...
import { FiChevronLeft, FiChevronRight, FiTrash2, FiPlus, FiEdit, FiSearch, FiMenu, FiFilter } from 'react-icons/fi';
import Link from 'next/link';
import CreateMenuItemModal from './createMenuItemModal'
import EditMenuItemModal from './editMenuItemModal';
import BulkUpdateMenuItemsModal from './bulkUpdateMenuItemsModal';
import BulkCreateMenuItemsModal from './bulkCreateMenuItemsModal';
...
export default function AdminMenuItemsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/menu-items?page=${currentPage}&limit=${pageSize}${searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : ''
      }`, {
      headers: {
      ...
      .then(res => {
        if (!res.ok) throw new Error('Error al cargar elementos de menú');
        return res.json();
      })
      .then(response => {
        ...
        // Extraer IDs de restaurante únicos para cargarlos
        const restaurantIds = [...new Set(response.data.map((item: MenuItem) => item.restaurantId))];
        loadRestaurantNames(restaurantIds);
      })
  ...
  const loadRestaurantNames = async (restaurantIds: string[]) => {
    ...
    try {
      ...
      const restaurantsData = await Promise.all(promises);
      ...
      console.error('Error cargando nombres de restaurantes', err);
    }
  };
  ...
  const deleteSelected = async () => {
    ...
    const confirm = window.confirm(`¿Eliminar ${selectedIds.length} ítems?`);
    if (!confirm) return;
    ...
    try {
      ...
      if (res.ok) {
        // Recargar los datos para mantener la consistencia
        setCurrentPage(1); // Volver a la primera página
        setSelectedIds([]);
      } else {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(errorData.message || 'Error al eliminar ítems');
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="container mx-auto">
          <h1 className="text-3xl font-bold flex items-center">
            <FiMenu className="mr-3" /> Administración de Menú
          </h1>
          <p className="mt-2 opacity-90">Gestiona los elementos del menú para todos los restaurantes</p>
        </div>
      </motion.div>
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
            <form onSubmit={handleSearch} className="flex-grow max-w-lg">
              <div className="relative">
                <input
                  ...
                  placeholder="Buscar elementos de menú..."
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="bg-[#FF6F61] text-white px-4 py-2 rounded-lg shadow hover:bg-[#e95d53] transition-colors flex items-center"
              >
                <FiPlus className="mr-2" /> Nuevo ítem
              </button>
              <button
                onClick={() => setShowBulkUpdateModal(true)}
                className="bg-yellow-500 text-white px-4 py-2 rounded-lg shadow hover:bg-yellow-600 transition-colors flex items-center"
              >
                🛠️ Actualizar en lote
              </button>
              <button
        ...
        >
          {loading && menuItems.length === 0 ? (
            <div className="p-8 flex flex-col items-center justify-center">
              ...
              <p className="mt-4 text-gray-600">Cargando elementos del menú...</p>
            </div>
          ) : menuItems.length === 0 ? (
            <div className="p-8 text-center">
              <FiFilter className="mx-auto text-gray-400 text-4xl mb-3" />
              <h3 className="text-lg font-medium text-gray-900 mb-1">No se encontraron elementos</h3>
              <p className="text-gray-500">
                {searchTerm ? 'Intenta con otra búsqueda o' : 'Empieza'} añadiendo nuevos elementos al menú.
              </p>
            </div>
          ) : (
                  ...
                  <tr>
                    <th className="p-4 w-10">
                      <input
                        ...
                        onChange={e => {
                          if (e.target.checked) {
                            setSelectedIds(menuItems.map(item => item._id));
                          } else {
                            setSelectedIds([]);
                          }
                        }}
                    ...
                    <th className="p-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Precio</th>
                    <th className="p-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Categoría</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {menuItems.map((item: MenuItem, index) => (
                    ...
                    >
                      ...
                      <td className="p-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-800">Q{item.price.toFixed(2)}</div>
                      </td>
                      <td className="p-4 whitespace-nowrap">
                        {item.category ? (
                          <span className="px-2 py-1 bg-[#FFF0EF] text-[#FF6F61] rounded-full text-xs font-medium">
                            {item.category}
                          </span>
                        ) : '-'}
                      </td>
                      <td className="p-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-500">
                          {restaurants[item.restaurantId]?.name ||
                            <span className="text-gray-500">{item.restaurantId.substring(0, 6)}...</span>}
                        </div>
                      </td>
              ...
              <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-gray-700">
                    ...
                    de <span className="font-medium">{pagination.total}</span> elementos
                  </p>
                </div>
                <div>
      ...
      {showCreateModal && (
        <CreateMenuItemModal
          ...
          onCreated={() => {
            setCurrentPage(1); // recarga primera página para incluir el nuevo ítem
          }}
        />
      )}
      ...
      {editItemId && (
        <EditMenuItemModal
          ...
          onUpdated={() => {
            setEditItemId(null);
            setCurrentPage(1); // Recarga para ver cambios
          }}
        />
      )}

      {showBulkUpdateModal && (
        <BulkUpdateMenuItemsModal
          token={token}
          onClose={() => setShowBulkUpdateModal(false)}
          onUpdated={() => {
            setCurrentPage(1); // Para reflejar los cambios
          }}
        />
      )}
      {showBulkCreateModal && (
        <BulkCreateMenuItemsModal
          token={token}
